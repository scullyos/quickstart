...
- **bo-ep API (direct):** http://127.0.0.1:18001
...
| `bo-ep-ms`                 | Backoffice entrypoint API — what the SPA calls               |
...
3. `./scripts/sh/up.sh` — starts everything in dependency order:
   ...
   - person-ms / authorization-ms / bo-ep-ms come up healthy
   - elasticsearch + es-setup + kibana + kibana-setup + fluent-bit come up
   - scullyosapp comes up last (waits for bo-ep healthy)
...
### Enabling the Admin Chat panel (optional)

The Admin Chat panel uses an LLM to drive the agent. **It's optional.** The shipped `.env.example` sets `LLM_API_KEY` to a placeholder so the stack boots without any LLM credentials; in that state the chat panel stays hidden and the SPA shows a dismissible notice explaining how to enable it.
...
**Your key never leaves your machine.** It's read by your local `bo-ep-ms` container and used to call the LLM provider directly. The platform does not phone home and does not relay your key anywhere.

After replacing the key, restart `bo-ep-ms`:

```bash
./scripts/sh/up.sh --force-recreate bo-ep-ms
```

To go back to the disabled state, restore the placeholder value (copy it back from `.env.example`) and restart `bo-ep-ms` the same way.
...
FLUENT_BIT_CONTAINER_INCLUDE_REGEX=^(person|authorization|bo-ep)-ms$|^my-app$|^worker-.+$
```

Restart fluent-bit for the change to take effect:

```bash
./scripts/sh/up.sh --force-recreate fluent-bit
```
...
docker compose logs -f bo-ep-ms

# Restart one service after editing its env file
./scripts/sh/up.sh --force-recreate bo-ep-ms

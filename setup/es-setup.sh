...
curl -s -u "$AUTH" \
    ...
    -H "Content-Type: application/json" \
    -d '{"index_patterns":["*-ms-*"],"template":{"mappings":{"properties":{"@timestamp":{"type":"date","format":"strict_date_optional_time||yyyy-MM-dd'\''T'\''HH:mm:ss.SSS||yyyy-MM-dd'\''T'\''HH:mm:ss.SSSZZ"},"ms":{"type":"keyword"},"message":{"type":"text"},"traceId":{"type":"keyword"},"spanId":{"type":"keyword"},"msreqid":{"type":"keyword"},"entityType":{"type":"keyword"},"errorStack":{"type":"text"},"context":{"type":"keyword"},"level":{"type":"keyword"}}}}}'
echo
